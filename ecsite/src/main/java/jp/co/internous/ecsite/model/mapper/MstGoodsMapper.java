...
public interface MstGoodsMapper {
	...
	List<MstGoods>findAll();/*←メソッドはこれです。コントローラクラス（インデックス）からこのメソッド呼び出されます。
	ただしコントローラークラスにインプリメンツがあるわけではない。なぜか？これがフレームワークなのだ。「こう書けばこう動く」だから。
	*/
...
/*インターフェイスを呼び出すクラスでインプリメンツせずともフレームワークであるマイバティスは、*/